...
class BookSearchController: UIViewController,UISearchResultsUpdating,UITableViewDelegate,UITableViewDataSource {
    ...
    func updateSearchResultsForSearchController(searchController: UISearchController) {
        ...
        if let tag = searchController.searchBar.text?.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceCharacterSet()) where !tag.isEmpty {
            //这里面需要向服务器请求数据,并刷新列表
            NetManager.getBooksTitles(tag, page: 0, resultClosure: { (titles) in
