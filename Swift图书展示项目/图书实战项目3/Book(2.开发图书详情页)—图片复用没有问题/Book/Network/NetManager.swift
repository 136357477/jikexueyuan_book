...
struct NetManager {
    ...
    /**
     ...
     - parameter resultClousure: <#resultClousure description#>
     */
    static func getReviewsWithBookId(bookId: String,page: Int,resultClousure:(Bool, [Review]!) -> Void){
        GET("https://api.douban.com/v2/book/\(bookId)/reviews", parameters: ["start":page * pageSize, "count":pageSize], showHUD: false, success: { (responseObject) in
            var reviews = [Review]()
            ...
            resultClousure(true, reviews)
            }) { (ErrorType) in
                resultClousure(false, nil)
        }
    }
